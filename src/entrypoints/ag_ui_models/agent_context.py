...
class AGUIAgentContext(AgentExecutionContext):
    ...
    def __init__(
        ...
        logger.info(f"Created AG-UI agent context: thread_id={thread_id}, run_id={self.run_id}, context_id={self.context_id}")
    ...
    async def send_status(self, message: str) -> None:
        ...
        try:
            ...
            if self.current_step:
                await self._emit_event(StepFinishedEvent(
            ...
            await self._emit_event(StepStartedEvent(
                type=EventType.STEP_STARTED,
                step_name=message
            ))
            
            # Also emit as custom event for detailed progress tracking
            await self._emit_event(CustomEvent(
                type=EventType.CUSTOM,
                name="status_update",
                value={"message": message, "timestamp": asyncio.get_event_loop().time()}
            ))
    ...
    async def send_response(self, response: str) -> None:
        ...
        try:
            ...
            # Start text message
            await self._emit_event(TextMessageStartEvent(
            ...
            await self._emit_event(TextMessageContentEvent(
            ...
            # End text message  
            await self._emit_event(TextMessageEndEvent(
